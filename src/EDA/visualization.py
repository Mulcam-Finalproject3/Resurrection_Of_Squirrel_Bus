...
def df_preprocess(df):
    ...
    df = df[df["시"] == "서울"]
    df_filtered = (
        df.groupby(["호선명", "지하철역", "시", "구", "동"])[["ride_sum", "alight_sum"]]
        .mean()
        .reset_index()
    )
    return df_filtered
...
def get_daram_95station_df():
    ...
    df_merged = pd.merge(
        df_daram, df_infra, left_on=["NODE_ID"], right_on=["NODE_ID"], how="left"
    )

    drop_col = ["정류소명_y", "X좌표_y", "Y좌표_y"]
    df_merged = df_merged.drop(drop_col, axis=1)
    df_merged.rename(
        columns={"정류소명_x": "정류소명", "X좌표_x": "X좌표", "Y좌표_x": "Y좌표"}, inplace=True
    )
...
def get_heatmap(data):
    ...
    sns.heatmap(data.corr(), annot=True, linewidths=0.5, annot_kws={"size": 7})
...
def get_barplot_daram_vs_all():
    ...
    final_tb_infra_population["NODE_ID"] = final_tb_infra_population["NODE_ID"].astype(
        "str"
    )
    ...
    final_tb_infra_population["법정동코드"] = final_tb_infra_population["법정동코드"].astype(
        "str"
    )
