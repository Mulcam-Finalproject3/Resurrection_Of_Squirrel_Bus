...
def get_population_data_by_bubjeong():
    # 행정-법정 테이블
    code = pd.read_excel('./csv/행정_법정코드_2021.xlsx', sheet_name='법정동_행정동코드')
    ...
    # 인구 테이블(통계청)
    df_pop = pd.read_csv('./csv/population_house_corp.csv')
    ...
    cal_col = ['tot_family','tot_ppltn','corp_cnt','employee_cnt','population_code_15to64','household_cnt_family','household_cnt_alone']
...
def preprocessing_infra():
    ...
    df = pd.read_csv('./csv/df_kakao_infra.csv')
    ...
    ## 대학교 infra 붙이기
    df_univ = pd.read_csv('./csv/university.csv')
    df = pd.merge(df, df_univ, left_on = '행정동',right_on='dong_name', how='left')
    df = df.fillna(0)
    df = df.drop('dong_name',axis=1)

    ## 대학병원 infra 붙이기
    df_univ_hos = pd.read_csv('./csv/university_hospital.csv')
    df = pd.merge(df, df_univ_hos, left_on = '행정동',right_on='dong_name',how='left')
    df = df.fillna(0)
    df = df.drop('dong_name',axis=1)
...
def get_final_infra_df():
    ...
    # emp_corp_ratio
    df_final['emp_corp'] = df_final['employee_cnt']/df_final['corp_cnt']
