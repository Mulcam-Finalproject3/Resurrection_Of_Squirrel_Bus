...
class RegressionTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self, target: str = None, rename_cols: list = None, drop_cols: list = None
    ):
        self.target = target
    ...
    def transform(self, dataframe):
        ...
        ohe_df = ohe_df.rename(
            columns={
                ohe_df.columns[0]: "six",
                ohe_df.columns[1]: "seven",
                ohe_df.columns[2]: "eight",
                ohe_df.columns[3]: "nine",
            }
        )
        ...
        standard_df = pd.DataFrame(
            self.scaler.fit_transform(standard_df), columns=standard_df.columns
        )
...
def XGB_grid_search(X_train, y_train):
    ...
    grid_xgb = GridSearchCV(
        xgb_model, param_grid=parameters, error_score="raise", n_jobs=-1, cv=3
    )
